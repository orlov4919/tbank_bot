...
import (
	...
	"linkTraccer/internal/infrastructure/siteClients/stackoverflow/mocks"
	"net/http"
	"net/url"
	"strings"
	"testing"
...
func TestStackClient_LinkState(t *testing.T) {
	...
	clientToGoodLinks.On("Do", mock.Anything).Return(&http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(bytes.NewBuffer([]byte(jsonAnswer)))}, nil)
	clientWithWrongBody.On("Do", mock.Anything).Return(&http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(bytes.NewBuffer([]byte(wrongJsonAnswer)))}, nil)
	clientToWrongLinks.On("Do", mock.Anything).Return(&http.Response{StatusCode: http.StatusNotFound, Body: io.NopCloser(nil)}, nil)
