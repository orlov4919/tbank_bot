...
func TestLinkHandler_GetMethodHandler(t *testing.T) {
	...
	for _, test := range tests {
		...
		if test.correct {
			...
			json.NewDecoder(w.Body).Decode(listLinksResponse)
...
func TestLinkHandler_PostMethodHandler(t *testing.T) {
	...
	userRepo.On("TrackLink", goodID, newLink).Return(nil)
	...
	for _, test := range tests {
		...
		if test.correct {
			...
			json.NewDecoder(w.Body).Decode(linksResponse)
	...
	tests := []testCase{
		...
		},
		{
			...
			req: &http.Request{Method: http.MethodGet,
				...
				Header: map[string][]string{"Tg-Chat-Id": []string{errIDStr}}},
			httpStatus: http.StatusBadRequest,
		},
		{
