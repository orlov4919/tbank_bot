...
func TestUserStorage_TrackLink(t *testing.T) {
	...
	for _, test := range tests {
		err := userRepo.TrackLink(context.Background(), test.userID, test.link, test.addTime)
...
func TestUserStorage_ChangeLastCheckTime(t *testing.T) {
	...
	for _, data := range dataToDB {
		err := userRepo.TrackLink(context.Background(), data.userID, data.link, time.Now())
...
func TestUserStorage_UsersWhoTrackLink(t *testing.T) {
	...
	for _, test := range tests {
		users, err := userRepo.UsersWhoTrackLink(test.linkID)
...
func TestUserStorage_DeleteUser(t *testing.T) {
	...
	for _, test := range tests {
		err := userRepo.DeleteUser(context.Background(), test.userID)
...
func TestUserStorage_DeleteUntrackedLinks(t *testing.T) {
	...
	tests := []TestCase{
		{
			name:        "firstID перестает отслеживать githubLink, после чего никто за ней не следит",
			userID:      firstID,
			deletedLink: githubLink,
			inLinks:     false,
		},
		{
			name:        "secondID перестает отслеживать stackoverflowLink, после чего firstID еще за ней следит",
			userID:      secondID,
			deletedLink: stackoverflowLink,
			inLinks:     true,
		},
		{
			name:        "firstID перестает отслеживать stackoverflowLink, после чего за ней никто не следит",
			userID:      firstID,
			deletedLink: stackoverflowLink,
			inLinks:     false,
		},
	...
	for _, test := range tests {
		...
		err = userRepo.DeleteUntrackedLinks()
